...
def load_prompt(prompt_fname):
    ...
    curr_dir = os.path.dirname(os.path.abspath(__file__))  # 获得当前文件所在目录的绝对路径
...
def normalize_vector(x, eps=1e-6):
    """normalize a vector to unit length"""     # 将向量归一化为单位长度
    x = np.asarray(x)
    if x.ndim == 1:
        norm = np.linalg.norm(x)   # 计算L2范数
        return np.zeros_like(x) if norm < eps else (x / norm)
    elif x.ndim == 2:
        norm = np.linalg.norm(x, axis=1)  # 按行计算L2范数
...
def calc_curvature(path):           # 计算路径曲率
