...
class LMP_interface():
    ...
    def __init__(self, env, lmp_config, controller_config, planner_config, my_world, env_name='isaac-sim'):
        self._env = env
        self._env_name = env_name
        self._cfg = lmp_config
        self._map_size = self._cfg['map_size']    # 200
        ...
        self._resolution = [x/self._map_size for x in self._env.work_space]
        # self._env.work_space / self._map_size
        print('#' * 50)
        print(f'## voxel resolution: {self._resolution}')
        print(f"map size:{self._map_size}")
        print('#' * 50)
        print()
        print()
    ...
    def detect(self, obj_name):
        if obj_name.lower() in EE_ALIAS:
            ...
            obs_dict['position'] = self.get_ee_voxel_pos()
            obs_dict['aabb'] = np.array([self.get_ee_voxel_pos(), self.get_ee_voxel_pos()])
            obs_dict['_position_world'] = self.get_ee_world_pos()
            # print(f"world ori:{self._env.get_ee_quat()}, local ori:{self._env.get_ee_local_quat()}")
            # input()
            ...
            voxel_map = self._points_to_voxel_map(obj_pc)                # 返回（map_size*3）数组， 其中obj对应区域值为1，其余区域值为0
            aabb_min = self._world_to_voxel(np.min(obj_pc, axis=0))      # 点云最小点和最大点，aabb用以记录区域的范围
    ...
    def execute(self, movable_obs_func, affordance_map=None, avoidance_map=None, rotation_map=None,
        ...
        Args:
        movable_obs_func: callable function to get observation of the body to be moved            # 即对应物体调用detect函数返回的obs类 
        ...
        object_centric = (not movable_obs_func()['name'] in EE_ALIAS)       # 为False表示是终端执行器
        execute_info = []
        if affordance_map is not None:
            ...
            for plan_iter in range(self._cfg['max_plan_iter']):
                ...
                movable_obs = movable_obs_func()         # 抓手gripper的detect 返回类 （可能是抓手gripper，也可能是类似抽屉手柄之类的）
                _affordance_map = affordance_map()       # map: 感兴趣区域（bin或这rubbish）为1，其余地方值均为零，对bin（target）仅中心点10cm高度处为1
                _avoidance_map = avoidance_map()         # 障碍物区域设置为1，其余位置均为0
                _rotation_map = rotation_map()           #（ms,ms,ms,4）,表征指定物体的旋转
                _velocity_map = velocity_map()           # 默认均为1，可以指定其x，y，z方向的速度因子
                _gripper_map = gripper_map()             # 数值0，1， 1表示抓手打开。0表示抓手闭合
                # preprocess avoidance map
                _avoidance_map = self._preprocess_avoidance_map(_avoidance_map, _affordance_map, movable_obs) # 返回的avoid map中障碍物位置及目标位置均置为1，目标周围区域为0
                # start planning
                ...
                if self._cfg['visualize'] and self._env.visualizer is not None:
                    ...
                    print(f"Do not visualize the planning path and points, if you want set the visualizer in {bcolors.OKBLUE}main.py{bcolors.OKBLUE} not None.")
                ...
                for i, waypoint in enumerate(traj_world):
                    ...
                    # loggging
                    movable_obs = movable_obs_func()                  # 每执行一个点后即调用detect更新场景状态
                    dist2target = np.linalg.norm(movable_obs['_position_world'] - traj_world[-1][0])
                    if not object_centric and controller_info['mp_info'] == -1:      # 根据action返回值判断路径执行过程是否中断
        ...
        if not object_centric:
            try:
                ...
                ee_pos_voxel = self.get_ee_voxel_pos()
            ...
            print(f"{bcolors.OKBLUE}[interfaces.py | {get_clock_time()} reach the target position {bcolors.ENDC}]")
    ...
    def _preprocess_avoidance_map(self, avoidance_map, affordance_map, movable_obs):
        # collision avoidance
        scene_collision_map = self._get_scene_collision_voxel_map()       # 获得场景voxel map ，不包含robot区域，全场景val均为1
        ...
        scene_collision_map[ignore_mask < int(0.15 * self._map_size)] = 0    # 在scene map中将感兴趣区域（bin）及冗余区域置为0
        # anywhere within 15/100 indices of the start is ignored
        try:
            ignore_mask = distance_transform_edt(1 - movable_obs['occupancy_map'])   # 对于gripper不存在map，其他物品才会有map
    ...
    def _get_scene_collision_voxel_map(self):
        collision_points_world, _ = self._env.get_scene_3d_obs(ignore_robot=True)     # 场景点云，env中搭建时并未设置忽略robot（存疑）
    ...
    def set_voxel_by_radius(self, voxel_map, voxel_xyz, radius_cm=0, value=1):
        ...
        if radius_cm > 0:
            radius_x = self.cm2index(radius_cm, 'x')   # 根据指定方向上的分辨率将指定的范围距离转化为像素点个数
    ...
    def _path2traj(self, path, rotation_map, velocity_map, gripper_map):
        ...
        for i in range(len(path)):
            ...
            voxel_xyz = path[i]
            # voxel_xyz = np.array([voxel_xyz[0], voxel_xyz[1], voxel_xyz[2]]) + np.array([0., 0., 0.0515/2])
            # get the current world position
            world_xyz = self._voxel_to_world(voxel_xyz)
            world_z = world_xyz[2] - np.array(0.02)
            world_xyz = np.array([world_xyz[0], world_xyz[1], world_z])
    ...
    def get_ee_world_pos(self):
        ee_world_pos = self._env.get_ee_pos()
        return ee_world_pos
    ...
    def _points_to_voxel_map(self, points):    # points in the world frame
        """convert points in world frame to voxel frame, voxelize, and return the voxelized points"""
        _points = points.astype(np.float32)
        _voxels_bounds_robot_min = self._env.workspace_bounds_min.astype(np.float32)   # coord in world frame
        _voxels_bounds_robot_max = self._env.workspace_bounds_max.astype(np.float32)
        _map_size = self._map_size
...
def setup_LMP(env, general_config, my_world, debug=False):
    ...
    # LMP env wrapper
    lmp_env = LMP_interface(env, lmp_env_config, controller_config, planner_config, my_world, env_name=env_name)
...
def pc2voxel_map(points, voxel_bounds_robot_min, voxel_bounds_robot_max, map_size):
  ...
  # voxelize
  voxel_xyz = (points - voxel_bounds_robot_min) / (voxel_bounds_robot_max - voxel_bounds_robot_min) * (map_size - 1)  # (N,3)
  # to integer
