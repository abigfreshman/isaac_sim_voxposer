...
class LMP:
    ...
    def __init__(self, name, cfg, fixed_vars, variable_vars, debug=False, env='rlbench'):
        self._name = name                   # “parse_query_obj” or“get_affordance_map” 等等，每一个功能对应一个LMP
        self._cfg = cfg
        self._debug = debug
        self._base_prompt = load_prompt(f"{env}/{self._cfg['prompt_fname']}.txt")  # 读取对应任务对应的提示
        self._stop_tokens = list(self._cfg['stop'])   # 指定读取tokens的停止位置
        self._fixed_vars = fixed_vars
        self._variable_vars = variable_vars
        self.exec_hist = ''
        self._context = None
        self._cache = DiskCache(load_cache=self._cfg['load_cache'])   # 加载缓存
    ...
    def build_prompt(self, query):
        ...
        if self._cfg['include_context']:
            ...
            prompt += f'\n{self._context}'                        # 场景中所有物品

        user_query = f'{self._cfg["query_prefix"]}{query}{self._cfg["query_suffix"]}'   # qp和qs分别表征query的开始和结束
        prompt += f'\n{user_query}'
    ...
    def _cached_api_call(self, **kwargs):
        ...
        if kwargs['model'] != 'gpt-3.5-turbo-instruct' and \
            ...
            new_query = '# Query:' + user1.split('# Query:')[-1]      # 得到最后一个query
            ...
            if kwargs in self._cache:                  # 如果任务已加载并保存，再次调用会优先读取已保存文件
                print('(using cache)', end=' ')
                return self._cache[kwargs]
            else:
                ret = openai.ChatCompletion.create(**kwargs)['choices'][0]['message']['content']   # GPT响应
                # post processing
    ...
    def __call__(self, query, **kwargs):
        ...
        while True:
            try:
                code_str = self._cached_api_call(            # 返回GPT生成的代码
                    prompt=prompt,
                    stop=self._stop_tokens,                  # 指定读取query的停止位置
                    temperature=self._cfg['temperature'],    # temperature = 0
                    model=self._cfg['model'],                # 指定大模型类型GPT-4
                    max_tokens=self._cfg['max_tokens']       # max_token=512
                )
                break
        ...
        to_log_pretty = highlight(to_log, PythonLexer(), TerminalFormatter())      # 高亮显示
        ...
        if self._debug:
            ...
            try:
                for s in action_str:
                    exec_safe(to_exec.replace(s, f'# {s}'), gvars, lvars)       # 调用python内置函数，执行GPT生成代码
        ...
        self.exec_hist += f'\n{to_log.strip()}'     # 存放历史exec运行的信息

        if self._cfg['maintain_session']:           # all False
            self._variable_vars.update(lvars)

        if self._cfg['has_return']:               # high ui(composer and planer) is False but lower ui(value map etc.) is True
            if self._name == 'parse_query_obj':
                try:
                    ...
                    return IterableDynamicObservation(lvars[self._cfg['return_val_name']])  # self._cfg['return_val_name']= ret_val
...
def exec_safe(code_str, gvars=None, lvars=None):
    ...
    if lvars is None:
        lvars = {}
    empty_fn = lambda *args, **kwargs: None      # 置为None
    custom_gvars = merge_dicts([
        gvars,
        {'exec': empty_fn, 'eval': empty_fn}
    ])
    try:
        exec(code_str, custom_gvars, lvars)       # 执行GPT生成的代码
    except Exception as e:
