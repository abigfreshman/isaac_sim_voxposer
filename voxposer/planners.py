...
class PathPlanner:
    ...
    def optimize(self, start_pos: np.ndarray, target_map: np.ndarray, obstacle_map: np.ndarray, object_centric=False):
        ...
        # smoothing
        target_map = distance_transform_edt(1 - target_map)     # 距离表征
        target_map = normalize_map(target_map)                  # distance to ->[0,1]
        obstacle_map = gaussian_filter(obstacle_map, sigma=self.config.obstacle_map_gaussian_sigma)   # sigma=10
        obstacle_map = normalize_map(obstacle_map)
        # combine target_map and obstacle_map
        costmap = target_map * self.config.target_map_weight + obstacle_map * self.config.obstacle_map_weight   # 2：1
        costmap = normalize_map(costmap)
        _costmap = costmap.copy()
        # get stop criteria      获得停止条件
        ...
        for i in range(self.config.max_steps):     # max_steps = 300
            # calculate all nearby voxels around current position
            all_nearby_voxels = self._calculate_nearby_voxel(current_pos, object_centric=object_centric)   # (N,3)
            # calculate the score of all nearby voxels
            nearby_score = _costmap[all_nearby_voxels[:, 0], all_nearby_voxels[:, 1], all_nearby_voxels[:, 2]]  # (N,)
        ...
        info['costmap'] = costmap                     # 由鼓励map和阻碍map共同组成的最初的costmap
        info['costmap_altered'] = _costmap            # 初始costmap经过路径检索后
    ...
    def _get_stop_criteria(self):
        def no_nearby_equal_criteria(current_pos, costmap, stop_threshold):    # st=0.001
            """
            ...
            nearby_locs = self._calculate_nearby_voxel(current_pos, object_centric=False)                     # 获得当前位置点附近一定区域内所有坐标点
    ...
    def _calculate_nearby_voxel(self, current_pos, object_centric=False):
        ...
        all_nearby_voxels = np.clip(current_pos + offsets_grid, 0, self.map_size - 1)   # 相对坐标转化为voxel中的坐标
    ...
    def _postprocess_path(self, path, raw_target_map, object_centric=False):
        ...
        """
        # smooth the path
        savgol_window_size = min(len(path), self.config.savgol_window_size)            # aavgol= 20
        savgol_polyorder = min(self.config.savgol_polyorder, savgol_window_size - 1)   # savgol_polyorder: 3
        path = savgol_filter(path, savgol_window_size, savgol_polyorder, axis=0)       # 施加平滑滤波
        # early cutoff if curvature is too high
        curvature = calc_curvature(path)         # 返回路径曲率数组
        if len(curvature) > 5:
            high_curvature_idx = np.where(curvature[5:] > self.config.max_curvature)[0]      #  max_curvature: 3
        ...
        if len(path_trimmed) > 1:
            target_spacing = int(self.config['target_spacing'] * self.map_size / 100) # 2        # target_spacing: 1
            length = np.linalg.norm(path_trimmed[1:] - path_trimmed[:-1], axis=1).sum()   # 实现path_trimmed中的后一个路径点减去前一个，得到相邻路径点之间的差值。范数计算得到总差值距离
            if length > target_spacing:
                ...
                if skip_ratio > 1:
                    path_trimmed = path_trimmed[::skip_ratio]      # 路径点下采样，采样间隔为skip_ratio
        ...
        if raw_target_map[last_waypoint[0], last_waypoint[1], last_waypoint[2]] == 0:       # 不在目标点处。rtm中val为1的位置表示感兴趣区域
        ...
        if object_centric:
            k = self.config['pushing_skip_per_k']                      #   pushing_skip_per_k: 5
