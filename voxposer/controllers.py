...
class Controller:
    ...
    def _calculate_ee_rot(self, pushing_dir):
        ...
        rotmat[:3, 1] = left
        rotmat[:3, 2] = up
        quat_wxyz = mat2quat(rotmat)     # 旋转矩阵转四元数
        return quat_wxyz
    ...
    def _apply_mpc_control(self, control, target_velocity=1):
        ...
        ee_quat = self._calculate_ee_rot(pushing_dir)    # 四元数
    ...
    def execute(self, movable_obs, waypoint):
        ...
        if not object_centric:                     # 终端执行器
            target_pose = np.concatenate([target_xyz, target_rotation])
            result = self.env.apply_action(np.concatenate([target_pose, [target_gripper]]))   # apply_action 返回obs，reward，flag（表征任务是否中断）
            info['mp_info'] = result
        # optimize through dynamics model to obtain robot actions
            ...
            self.mpc_velocity = target_velocity    #  target_velocity=1 或0.5， 1.5
    ...
    def random_shooting_MPC(self, start_obs, target):
        ...
        for t in range(self.config.horizon_length):   # hl = 1
            ...
            pred_next_obs = self.forward_step(curr_obs, controls)                       # 返回运动一步后的点云状态
            # record the control sequence and the resulting observation
            obs_sequences.append(pred_next_obs)  # obs_sequences: [T, N, obs_dim]
            controls_sequences.append(controls)  # controls_sequences: [T, N, control_dim]    # [T,B,7]
